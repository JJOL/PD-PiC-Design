...
class Element(Statement):
    ...
    def __init__(self, line):
        ...
        # print("Parsing an element with prefix '{}'".format(self._prefix))
        # print("Prefix data: ", prefix_data)
...
class EBeamElement(Statement):

    """Parse SiEPIC/EBeam-style instance lines.

    Expected format::

        instance node1 node2 ... model key=value key=value ...

    Example::

        ebeam_wg_integral_1550_4 N$0 N$2 ebeam_wg_integral_1550 library="Design kits/EBeam"
    """

    _logger = _module_logger.getChild('EBeamElement')
    ...
    def __init__(self, line):
        ...
        line_str = str(line)
        tokens = shlex.split(line_str)
        if len(tokens) < 4:
            raise ParseError('Bad EBeam element line: {}'.format(line_str))
        ...
        if len(positionals) < 2:
            raise ParseError('Bad EBeam element line, expected nodes and model: {}'.format(line_str))

        # EBeam lines use: instance <nodes...> <model> [key=value...]
        self._nodes = positionals[:-1]
        self._parameters = [positionals[-1]]

    ##############################################

    def __repr__(self):
        return 'EBeamElement {0._name} {0._nodes} {0._parameters} {0._dict_parameters}'.format(self)
    ...
    def to_python(self, netlist_name, ground=0):
        ...
        nodes = self.translate_ground_node(ground)
        args = self.values_to_python([self._name] + self._parameters + nodes)
        kwargs = self.kwargs_to_python(self._dict_parameters)
        return '{}.X({})'.format(netlist_name, self.join_args(args + kwargs)) + os.linesep
    ...
    def build(self, circuit, ground=0):
        ...
        nodes = self.translate_ground_node(ground)
        kwargs = self._dict_parameters
        circuit.X(self._name, *(self._parameters + nodes), **kwargs)
...
class SpiceParser:
    ...
    def _parse(self, lines, recurse=False, section=None):
        ...
        for line in lines[start_index:]:
            ...
            if skip_lines[-1]:
                ...
                try:
                    if 'library=' in lower_case_text:
                        try:
                            element = EBeamElement(line)
                        except ParseError:
                            # Fallback to default SPICE parser for non-EBeam lines using a library kwarg.
    ...
    def _build_circuit(circuit, statements, ground):
        ...
        for statement in statements:
            if isinstance(statement, (Element, EBeamElement)):
